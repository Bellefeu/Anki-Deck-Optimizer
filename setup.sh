...
#
#   ./setup.sh              install everything missing, then run bootstrap
#   ./setup.sh --dry-run    show exactly what it would run, change nothing
#   ./setup.sh --yes        do not ask before installing
#
...
# Python it needs to run. Everything after Python is handled here too, so the
# user only ever types one command.
PY=""
...
NEED=()
declare -A PROVIDES=(
  [pdftotext]=poppler [pdftoppm]=poppler [pdfinfo]=poppler [pdfimages]=poppler
  [tesseract]=tesseract [node]=node
)
for t in pdftotext pdftoppm pdfinfo pdfimages tesseract node; do
  if command -v "$t" >/dev/null 2>&1; then ok "$t"; else miss "$t  (from ${PROVIDES[$t]})"; NEED+=("${PROVIDES[$t]}"); fi
done
# de-duplicate
NEED=($(printf '%s\n' "${NEED[@]:-}" | sort -u | grep -v '^$' || true))

# ---------------------------------------------------------------- install
if [ -z "${NEED_PY:-}" ] && [ "${#NEED[@]}" = "0" ]; then
  echo; ok "everything is already installed"
else
  echo
  ...
  if [ "$PM" = "brew" ] && ! command -v brew >/dev/null 2>&1 && [ "$DRY" != "1" ]; then
    ...
    if confirm "Install Homebrew now? (asks for your Mac password)"; then
      ...
      echo "     Install Homebrew yourself: https://brew.sh   then re-run ./setup.sh"
      exit 1
    fi
  fi
  ...
  case "$PM" in
    brew)
      PKGS=(); [ -n "${NEED_PY:-}" ] && PKGS+=(python@3.12)
      for n in "${NEED[@]:-}"; do case "$n" in
        poppler) PKGS+=(poppler);; tesseract) PKGS+=(tesseract);; node) PKGS+=(node);; esac; done
      confirm "Install with Homebrew: ${PKGS[*]}" || { err "declined"; exit 1; }
      run brew install "${PKGS[@]}"
      ;;
    apt-get)
      PKGS=(); [ -n "${NEED_PY:-}" ] && PKGS+=(python3 python3-pip python3-venv)
      for n in "${NEED[@]:-}"; do case "$n" in
        poppler) PKGS+=(poppler-utils);; tesseract) PKGS+=(tesseract-ocr tesseract-ocr-eng);; node) PKGS+=(nodejs npm);; esac; done
      confirm "Install with apt (needs your password): ${PKGS[*]}" || { err "declined"; exit 1; }
      run sudo apt-get update -qq
      run sudo apt-get install -y "${PKGS[@]}"
      ;;
    dnf|zypper)
      PKGS=(); [ -n "${NEED_PY:-}" ] && PKGS+=(python3 python3-pip)
      for n in "${NEED[@]:-}"; do case "$n" in
        poppler) PKGS+=(poppler-utils);; tesseract) PKGS+=(tesseract tesseract-langpack-eng);; node) PKGS+=(nodejs);; esac; done
      confirm "Install with $PM (needs your password): ${PKGS[*]}" || { err "declined"; exit 1; }
      run sudo "$PM" install -y "${PKGS[@]}"
      ;;
    pacman)
      PKGS=(); [ -n "${NEED_PY:-}" ] && PKGS+=(python python-pip)
      for n in "${NEED[@]:-}"; do case "$n" in
        poppler) PKGS+=(poppler);; tesseract) PKGS+=(tesseract tesseract-data-eng);; node) PKGS+=(nodejs npm);; esac; done
...
[ -n "$PY" ] || { err "Python 3.10+ still not found after install. Open a NEW terminal and re-run ./setup.sh"; exit 1; }
echo

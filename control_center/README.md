...
- macOS: `Prism Control Center - Mac.command`
- Linux/terminal: `bash control_center/launch.sh`

The dashboard can stage source files and one `.apkg`, open their exact destination
folders, edit `PROFILE.md` and `USER_PROMPTS.md`, open deck review artifacts, launch
setup, and install stable releases. Resetting preferences requires two confirmations and
...
python3 control_center/build_manifest.py 1.2.0
...
semantic version, such as `v1.2.0`, then publish a non-draft, non-prerelease GitHub
...
For the next release, increase the version (for example, `1.2.1` for a bug fix or `1.3.0`
